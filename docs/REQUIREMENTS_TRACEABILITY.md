...
| REQ-CLI-001 | 11, 20 | Manifest-backed `forge graph apply` and `forge build` workflows | `include/forge/manifest.hpp`, `src/workspace/manifest.cpp`, `src/cli/cmd_graph.cpp`, `src/cli/cmd_build.cpp`, `examples/simple.fgm` | `ManifestApplyBuildsTargets`; Docker CLI smoke for `graph apply` and `build` passed | Verified | pending |
