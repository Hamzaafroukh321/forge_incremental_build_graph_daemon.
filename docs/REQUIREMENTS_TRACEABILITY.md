...
| REQ-FIPC-001 | 7, 8, 10, 11 | FIPC-1 frame codec with CRC, caps, streaming, malformed errors, Unix-domain status transport | `include/forge/fipc.hpp`, `include/forge/daemon.hpp`, `src/ipc/*` | `EveryByteFrameSplit`, `MalformedFrameRejected`, `UnixSocketDaemonStatusRoundTrip`, `forge_fipc_decoder_fuzz`; Docker debug/ASan/fuzz smoke pass | Verified | 54aa070, ca4c2e8, 5de0292 |
...
| REQ-FST-001 | 7, 10 | FST-1 canonical record writer/reader, filesystem append, committed-prefix replay, checkpoint rotation | `include/forge/state.hpp`, `src/state/*` | `CrashEveryStateRecordBoundary`, `BadCommitDigestIgnored`, `CheckpointLogRotationReplay`, `FileStateStoreReopensCommittedLog`, `FileStateStoreCompactsCrashTailBeforeAppend`, `FileStateStoreCheckpointRotatesLog`; Docker debug/release/ASan tests pass | Verified | 54aa070, ca4c2e8, 3e58530 |
...
| REQ-WORKSPACE-001 | 8, 10, 15 | Path normalization, coalescing, stale fingerprint rejection, Linux inotify event intake | `include/forge/workspace.hpp`, `src/workspace/*` | `UnchangedContentNoInvalidation`, `StaleFingerprintIgnored`, `InotifyWatcherReportsFileChange`; Docker debug/release/ASan tests pass | Verified | 54aa070, c10b054 |
...
| REQ-CLI-001 | 11, 20 | Manifest-backed `forge graph apply`/`forge build` workflows and socket-backed `forge status` | `include/forge/manifest.hpp`, `src/workspace/manifest.cpp`, `src/cli/cmd_graph.cpp`, `src/cli/cmd_build.cpp`, `src/cli/cmd_status.cpp`, `src/cli/forged_main.cpp`, `examples/simple.fgm` | `ManifestApplyBuildsTargets`, `UnixSocketDaemonStatusRoundTrip`; Docker CLI smoke for `graph apply`, `build`, and `forged --socket` plus `forge status --socket` passed | Verified | 0774bb3, 5de0292 |
...
| REQ-CAS-001 | 4, 8, 10, 15 | Artifact digest verification, publication, leases, filesystem reopen, GC | `include/forge/artifact.hpp`, `src/artifact/*` | `ArtifactDigestMismatchQuarantined`, `MultiArtifactCommitAtomic`, `CasGcKeepsLeases`, `DiskArtifactPublishReopens`; Docker debug/release/ASan tests pass | Verified | 54aa070, 5022691 |
...
| REQ-RECOVERY-001 | 4, 7, 15 | Restart from committed checkpoint/log and mark running attempts lost | `src/state/recovery.cpp`, `src/state/file_state_store.cpp`, `include/forge/state.hpp` | `MissingCasBlocksRecoveredSuccess`, `WatcherRootRescanAfterRestart`, `BadCommitDigestIgnored`, `FileStateStoreReopensCommittedLog`, `FileStateStoreCompactsCrashTailBeforeAppend`, `FileStateStoreCheckpointRotatesLog`; Docker debug/release/ASan tests pass | Verified | 54aa070, ca4c2e8, 3e58530 |
